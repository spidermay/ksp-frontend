...
@Component({
    selector:"list_component-beneficiary",
...
export class ListComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.TranslateService.get(["SUCCESS","ERRORS","LIST","BUTTONS","PROFILE","DATE","ADDRESS"]).subscribe((Translation:string) => { this.Translation = Translation; });
        this.Table.init();
    }

    ngOnInit(){
        // console.log(this.data);
        (this.data.beneficiary_id)? this.Filters.beneficiary_id = this.data.beneficiary_id : false;
        ...
        this.ServiceBeneficiary.data_emmiter.subscribe( return_data => {
    ...
    ngOnDestroy(){
        // this.ServiceBeneficiary.data_emmiter.unsubscribe();
    }

    async loadData(){
        this.Utils.showSpiner();
        this.ServiceBeneficiary.getList(this.Filters).subscribe((return_data:any) => {
            // console.log(return_data);
            this.Utils.closeSpiner();

            if(return_data.success == 1){
                this.ServiceBeneficiary.data_emmiter.emit({ action:"summary", data:return_data.summary });
    ...
    download(type?:string){
        ...
        var column_data = {
			...
			fullname:this.Translation["PROFILE"]["BENEFICIARY"],
			birthday:this.Translation["PROFILE"]["BIRTHDAY"],
			relationship:this.Translation["PROFILE"]["RELATIONSHIP"],
			employee_fullname:this.Translation["PROFILE"]["EMPLOYEE"],
		};

        this.Utils.showSpiner();
        
        if(type == "csv"){
            this.ServiceBeneficiary.getList(data).subscribe((return_data:any) => {
                // console.log(return_data);
                this.Utils.closeSpiner();   
                this.Utils.downloadFileCSV(return_data.data, "list-beneficiary", column_data);    
            }, error => {
