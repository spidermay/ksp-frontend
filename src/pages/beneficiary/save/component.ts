...
export class SaveComponent implements OnInit{
    ...
    public url = Config.api_url + "beneficiary/storage/";
    ...
    constructor(
        ...
        this.TranslateService.get(["SUCCESS","ERRORS","BUTTONS","LIST","LANGUAGE","PROFILE"]).subscribe((Translation:string) => { this.Translation = Translation; });
    ...
    ngAfterViewInit(){
        ...
        $("#birthday").datetimepicker({ format: "DD-MM-YYYY" });
    }
    
    async loadData(){
        await this.ServiceBeneficiary.getProfile({ item_id:this.item_id, loading:true }).subscribe((return_data:any) => {
            ...
            if(return_data.success == 1){
                ...
                $("#sex option[value='" + this.Row.data.beneficiary.sex + "']").attr("selected", "selected");
                $(".selectpicker").selectpicker("refresh");      
            } else {
                console.log(return_data);
    ...
    save(form:NgForm) {
        ...
        if(form.valid) { 
            ...
            let data = this.Row.getDataValues();
                ...
                // (data["employee_id"] == undefined || data["employee_id"] < 1)? data["employee_id"] = 1 : false ;
    ...
    private sendData(data):void {
        ...
        this.ServiceBeneficiary.save(data).subscribe((return_data:any) => {
            // console.log(return_data);
            ...
            if(return_data.success == 1){
                ...
                swal({
                    ...
                    (value == "view")? this.Router.navigate(["beneficiary/view", { item_id:this.item_id }]) :false;
                    (value == "next")? this.Router.navigate(["beneficiary/save", { item_id:return_data.pager.next }]) :false;
                    (value == "new") ? this.Router.navigate(["beneficiary/save", { item_id:0 }]) :false;
                    (value == "list")? this.Router.navigate(["beneficiary/list"]) :false;
                });
            } else {
    ...
    onChangeFilterEmployee(value){
        this.Row.data.beneficiary.employee_id = value;
    }

    async loadFilterEmployee(item_id?, id = "employee_id"){
		this.ServiceEmployee.getPair({ token:this.token, recursive:true, active:1, deleted:1, limit:500 }).subscribe(async (return_data:any) => {
            // console.log(return_data);
            ...
            if(return_data.success == 1){
                ...
                $("#" + id).find("option").remove().end().append(select_options).selectpicker("refresh");

                if(item_id >= 0){
					$("#" + id + " option[value='" + item_id + "']").attr("selected","selected");
