...
@Component({
    ...
    selector: "select_component-beneficiary",
})
...
export class SelectComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
		...
		this.data.id = (this.data.id)? this.data.id : "beneficiary_id";
		this.data.item_id = (this.data.item_id)? this.data.item_id : "";
		this.data.label = (this.data.label)? this.data.label : this.Translation["PROFILE"]["PROMOTER"];
	...
	async loadData(){
		await this.ServiceBeneficiary.getList(this.Filters).subscribe(async (return_data:any) => {
            // console.log(return_data);
            if(return_data.success == 1){
                var select_options = "<option value=''>" + this.Translation["LIST"]["ALL"] + "</option>";

				$.each(return_data.data, function (key, value) {
                    select_options += "<option value='" + value.item_id + "'>" + value.names + "</option>"; 
                });
